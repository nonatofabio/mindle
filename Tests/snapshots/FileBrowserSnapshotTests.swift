...
struct FileBrowserSnapshotTests {
    ...
    private static func render(theme: ReaderTheme, state: SnapshotState) throws -> Data {
        ...
        let newFile = root.appendingPathComponent("meeting-notes.txt")
        let rows = [
            FileTreeRowModel(
                url: chapters,
                name: "Chapters",
        ...
        let metadata = GitMetadataSnapshot(files: [
            active: GitFileMetadata(
                changes: GitFileChanges(additions: 12, deletions: 3, isUntracked: false),
                lastEditedAt: fixedNow.addingTimeInterval(-8 * 86_400)
            ),
            draft: GitFileMetadata(
                changes: nil,
                lastEditedAt: fixedNow.addingTimeInterval(-65 * 86_400)
            ),
            newFile: GitFileMetadata(
                changes: GitFileChanges(additions: nil, deletions: nil, isUntracked: true),
                lastEditedAt: nil
            )
        ])
        let tree = FileNode(
        ...
        let view = FileBrowserSidebarContent(
            rootDisplayName: "Field Notes",
            rootURL: root,
            tree: state == .error ? nil : tree,
            rows: state == .populated ? rows : [],
            selectedURL: active,
            isLoading: false,
            errorMessage: state == .error ? "The SSH connection timed out." : nil,
